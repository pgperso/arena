      ...
      <div className="overflow-hidden rounded-2xl bg-gray-950">
        ...
        {allMessages.length > 1 && (
          <div className="flex gap-1 px-5 pb-4">
            {allMessages.map((_, i) => (
              <div key={i} className="h-0.5 flex-1 overflow-hidden rounded-full bg-white dark:bg-gray-900/5">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${
                    i === currentIndex ? 'w-full bg-brand-orange' : i < currentIndex ? 'w-full bg-white dark:bg-gray-900/10' : 'w-0'
                  }`}
                />
              </div>
            ))}
